...
if(empty($_SESSION['user'])) {		
	...
	try {
		 ...
		 $query = "INSERT INTO lineup(id, user_id, contest_id, remain_salary, fpts_total) VALUES ('', ?, ?, ?, ?)";
		 $q = mysqli_stmt_init($dbcon);
		 mysqli_stmt_prepare($q,$query);
		 mysqli_stmt_bind_param($q, 'iisd', $user_id, $contest_id, $sal_remain, $fpts_total); 
		 mysqli_stmt_execute($q);
		 
		 $lineup_create = mysqli_stmt_affected_rows($q);
		 echo'<script>console.log('.$lineup_create.');   </script>';
		 ...
		 if (mysqli_stmt_affected_rows($q) == 1) {
			...
			$query = "INSERT INTO lineup_position(lineup_id, p_position, p_id, p_name, opponent,
					   p_fppg, p_salary) 
					   VALUES (?,'PG',?,?,?,?,?), (?,'SG',?,?,?,?,?), (?,'SF',?,?,?,?,?),
					   (?,'PF',?,?,?,?,?), (?,'C',?,?,?,?,?), (?,'G',?,?,?,?,?),
					   (?,'F',?,?,?,?,?), (?,'UTIL',?,?,?,?,?)";
			...
			if (mysqli_stmt_affected_rows($q) >= 1) { 
			  ...
			  echo ' <script>
						...
						lineup_alert.render("Lineup created successfully.");	
					 </script> ';	
				 ...
				 echo ' <script>
							var lineup_alert = new CustomAlert();
							lineup_alert.render("Lineup was not created. Please try again.");	
						 </script> ';			 
			}
		 } else {
			  ...
			  echo ' <script>
					...
					cust_alert.render("The system is busy. Please try saving your lineup again later.");	
				</script>     ';			 
		 }	
		 mysqli_close($dbcon); 
	}
